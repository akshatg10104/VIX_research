...
The remainder of the paper is organized as follows. Section~\ref{sec:related} reviews related work. Section~\ref{sec:data} describes the data. Section~\ref{sec:methodology} presents the methodology. Section~\ref{sec:results} reports empirical results, including persistence quantification (Section~\ref{sec:persist_quant}), regime-transition case studies (Section~\ref{sec:transitions}), LSTM comparison (Section~\ref{sec:lstm}), and SHAP attribution (Section~\ref{sec:shap}). Section~\ref{sec:replication} replicates the diagnostic on S\&P 500 trend regimes. Section~\ref{sec:conclusion} concludes.
...
The LSTM achieves lower selective accuracy than the RF at both horizons. On the regime-transition audit, the LSTM shows non-zero calm$\to$high covered accuracy (34.9\% at $N = 5$; 19.1\% at $N = 10$)---unlike the RF's 0\%---because the LSTM's calibrated probabilities are less tightly clustered near the prediction boundaries, so it makes predictions on more uncertain days including some transition days. However, 34.9\% and 19.1\% are both substantially below the 50\% random-chance baseline, meaning the LSTM is confidently wrong more often than right when it does predict on transition days. This broader coverage of transition days comes entirely at the expense of precision on stable days: calm$\to$calm covered accuracy is 83.1\% at $N = 5$ compared to the RF's 99.6\%. The LSTM's overall lower accuracy reflects this tradeoff.
...
Permutation importance quantifies the marginal accuracy drop from shuffling each feature; SHAP (SHapley Additive exPlanations) values provide a complementary view by decomposing each individual prediction into additive feature contributions grounded in cooperative game theory \citep{lundberg2017}. We compute SHAP values using \texttt{TreeExplainer} on the test set at $N = 5$ and $N = 10$.
...
VIX technical features account for \textbf{71.9\%} of total SHAP mass at $N = 5$ and \textbf{64.7\%} at $N = 10$; S\&P 500 features account for 19.2\% and 21.3\% respectively; macro and cross-asset features together account for under 10\% at both horizons. The dominant features---VIX\_SMA10, VIX\_SMA20, VIX\_BB\_UPPER, VIX\_BB\_LOWER---are all smoothed or envelope transforms of the current VIX level. SHAP therefore confirms and quantifies what the persistence analysis (Section~\ref{sec:persist_quant}) established: the model's predictions are driven overwhelmingly by VIX autocorrelation, with a secondary role for equity-market stress indicators and negligible contribution from the broader cross-asset and macro feature set. The 33 additional features beyond the three-variable HAR structure contribute only 28\% of SHAP mass combined.
...
The VIX findings raise a natural question: is the persistence illusion specific to volatility forecasting, or is it a general property of selective prediction on any autocorrelated regime label? To answer this, we replicate the full diagnostic protocol on a structurally different regime classification task using the same feature set and model specification.

\subsection{Setup}

The replication target is \textbf{S\&P 500 trend-regime classification}: at time $t$, the label is
\begin{equation}
    \text{BEAR}_{N,t} = \begin{cases} 1 & \text{if } \text{SP500}_{t+N} < \overline{\text{SP500}}_{200,t+N} \\ 0 & \text{otherwise} \end{cases}
\end{equation}
where $\overline{\text{SP500}}_{200,t+N}$ is the 200-day moving average of the S\&P 500 at time $t + N$. This label captures whether the market will be in a downtrend (below its long-run average) $N$ days ahead. It is structurally different from the VIX label---equity trend vs.\ implied volatility level---but shares the key property of strong autocorrelation: equity trends are persistent. The bear-market base rate in the test set ($\approx$22\%) is comparable to the VIX high-volatility base rate ($\approx$30\%), and the same 36 features, same RF specification, and same $\tau = 0.25$ selective threshold are used throughout. The HAR-analog baseline uses the S\&P 500 level, its 5-day mean, and its 20-day mean---the equity counterpart of the VIX HAR.
...
\begin{figure}[H]
    ...
    \caption{S\&P 500 trend-regime replication. Left: RF vs.\ persistence vs.\ HAR-analog accuracy---persistence dominates at all horizons. Center: persistence ratio P(bear$\mid$bear)/P(bear$\mid$bull), which is 23.8$\times$ at $N = 5$ (higher than VIX's 9.7$\times$). Right: transition accuracy---bull$\to$bear covered accuracy is 0\% at both horizons, replicating the VIX calm$\to$high finding exactly.}
    \label{fig:sp500}
\end{figure}

The replication results match the VIX findings in every qualitative respect and strengthen them quantitatively. The persistence ratio is \textbf{23.8$\times$} at $N = 5$---more than twice the VIX ratio of 9.7$\times$---reflecting the well-known long memory of equity trends. As a result, persistence at matched coverage achieves 100\% accuracy at $N = 5$ and beats the RF selective classifier at \emph{every horizon} (RF$-$Persist ranges from $-4.0$ to $-7.1$ pp). Bull$\to$bear covered accuracy is \textbf{0.0\%} at both $N = 5$ and $N = 10$---exactly replicating the VIX calm$\to$high finding---and 97--99\% of the RF's correct covered predictions are same-regime calls. Balanced accuracy collapses to 49.2\% at $N = 25$, again matching the VIX degeneration.
